...
C----------------------------------------------------------------------------
      ...
      !!
      !!    Calling the subroutines below in the original F1F209.f is very slow
      !!    because it initializes all the arrays for each call. This init subroutine 
      !!    should be called before using any of the subroutines below. 
      !! 
      !! Revision History:
      !!    - Added by Whitney Armstrong 10-20-2014
      SUBROUTINE INIF1F209
      IMPLICIT NONE
       real*8 xxp(99)/
     ...
     > 0.92047,0.92494,0.94450,0.94782,0.96768,0.96975,0.98974,1.0/
      ...
      common/F1F2COMM/xxp,fyp,xvald0,fyd,avp2
c      WRITE(*,*)'INIF1F209'

       real*8 fydd(20)/ 0.4965, 0.4988, 0.4958, 0.5008, 0.5027, 0.5041,
     >  0.5029, 0.5034, 0.4993, 0.5147, 0.5140, 0.4975, 0.5007, 0.4992,
     >  0.4994, 0.4977, 0.5023, 0.4964, 0.4966, 0.4767/
       real*8 avpzd(20)/-0.1820,-0.0829,-0.0590,-0.0448,-0.0345,-0.0264,
     > -0.0195,-0.0135,-0.0079,-0.0025, 0.0029, 0.0083, 0.0139, 0.0199,
     >  0.0268, 0.0349, 0.0453, 0.0598, 0.0844, 0.1853/
       real*8 avp2d(20)/ 0.0938, 0.0219, 0.0137, 0.0101, 0.0081, 0.0068,
...
c      real*8 fydfd(200)
c      real*8 avp2fd(200)
      common/F1F22COMM/fydd,avpzd,avp2d,fydfd,avp2fd
      RETURN
      END

C----------------------------------------------------------------------------

      !>  Quasi elastic contribution only F1F209 subroutine
      !! 
      !! D. Flay (July 2012) 
      !! Modified version of F1F209 to fit E94-010 and E01-012 data 
      !! for use as cross section input to radiative corrections for E06-014. 
      !!
      !! Whitney Armstrong 
      !!
      !! @param Z number of protons
      !! @param A atomic number
      !! @param Es beam energy (GeV)
      !! @param Ep scattered electron energy (GeV)
      !! @param TH scattered electron angle (radians)
      !! @param SIGMA cross section (nb/GeV/sr)
      !! @param f1_full F1 structure function including quasielastic
      !! @param f2_full F2 structure function including quasielastic
      !! @param q2 Q-squared
      !! @param wsq invariant mass squared
      !! 
      !! Revision History:
      !!    - Added by D. Flay - Calculates the total cross section
      !! 
      !!
      !! @ingroup F1F209 
      !!
      SUBROUTINE CROSS_QE_MOD(Z,A,Es,Ep,TH,SIGMA_TOT,F1Tot,F2Tot,Q2,Wsq)
      IMPLICIT NONE
      REAL*8 Mp
      REAL*8 A,Z
      REAL*8 PI/3.1415927/
      REAL*8 ALPHA/7.29735E-03/
      REAL*8 CHBAR2/0.389899/    ! units: GeV2*mb 
      REAL*8 SIN2,COS2,TAN2
      REAL*8 Es,Ep,Nu,x,Q2,W,Wsq
      REAL*8 DEG_TO_RAD,TH,thr
      REAL*8 F1IN,F2IN,RIN,F1QE,F2QE
      REAL*8 F1Tot,F2Tot,W1,W2 
      REAL*8 SIGMA_MOTT,SIGMA_IN,SIGMA_QE,SIGMA_TOT 
      REAL*8 NUM,DEN
 
C     Kinematics and constants
      Mp         = 0.9383
      DEG_TO_RAD = PI/180.       
      thr        = TH 
      SIN2       = SIN(thr/2.)**2
      ...
      Q2         = 4.*Es*Ep*SIN2
      x          = Q2/(2.0*Mp*Nu)
      Wsq        = Mp*Mp + 2.*Mp*Nu - Q2  
      W          = SQRT(Wsq)

C     Mott cross section 
      NUM        = CHBAR2*(1E+6)*(ALPHA**2)*COS2 ! CHBAR2 converts to nb  
      DEN        = (2.*Es*SIN2)**2
      SIGMA_MOTT = (NUM/DEN)   
C     Unpolarized structure functions    
      CALL F1F2QE09(Z,A,Q2,Wsq,F1QE,F2QE)
      !CALL F1F2IN09_MOD(Z,A,Es,Q2,Wsq,F1IN,F2IN,RIN)

C     Full unpolarized structure functions 
      F1Tot     = F1QE !+ F1IN 
      F2Tot     = F2QE !+ F2IN 
C     Total cross section 
      W2        = (F2Tot)/Nu
      W1        = (F1Tot)/Mp
      SIGMA_TOT = SIGMA_MOTT*(W2 + 2.*TAN2*W1)
...
C----------------------------------------------------------------------------
      ...
      !! @ingroup F1F209 
      !!
      SUBROUTINE CROSS_TOT_MOD(Z,A,Es,Ep,TH,SIGMA_TOT,F1Tot,F2Tot,Q2,Wsq)
      IMPLICIT NONE
      REAL*8 Mp
      REAL*8 A,Z
      REAL*8 PI/3.1415927/
...
C----------------------------------------------------------------------------
      ...
      SUBROUTINE F1F2IN09_MOD(Z,A,Es,QSQ,WSQ,F1,F2,RC)
      IMPLICIT NONE
      REAL*8 Z,A
      REAL*8 Es,QSQ,WSQ,F1,F2,RC   
      REAL*8 SF 
...
!> @defgroup F1F209 F1F209
!! 
!> @{

      !> Calculates the total cross sections
      !!
      !! @param Z number of protons
      !! @param A atomic number
...
CC      real*8 pi/3.1415927/, am/0.9383/, CHBAR/19732.8/      
      ...
      real*8 th_a,f1_full,f2_full,x_b
      th_a = th*180./pi
...
C----------------------------------------------------------------------------
      ...
      !! Energies are in GeV.
      !!
      SUBROUTINE cross_qe(Z,A,E0,EP,TH,SIGMA,f1_full,f2_full,q2,wsq)
      implicit none
      real*8 Z, A, E0, ep, th, sigma, F1, F2,sigmaqe,F1qe,F2qe
      real*8 sigmaIn,sigmaEl
CC      real*8 pi/3.1415927/, am/0.9383/, CHBAR/19732.8/      
...
C----------------------------------------------------------------------------
      ...
      !! 
      !! @param Z proton number of nucleus (use Z=0., A=1. to get free neutron)
      !! @param A atomic number of nucleus (use Z=0., A=1. to get free neutron)
      !! @param QSQ is 4-vector momentum transfer squared (positive in chosen metric)
      !! @param Wsq (real*8) is invarinat mass squared of final state 
      !! calculated assuming electron scattered from a free proton
      !! @param F1 structure function per nucleus
      !! @param F2 structure function per nucleus
      !! 
      !! REVISION HISTORY
      !! 
      !! - 10/20/2006 P. Bosted
      !!   Modified version by Peter
      !!   
      !! - 10/19/2011 Whitney Armstrong
      !!   Added documentation and hooks for linking into C/C++
      !! 
...
C This is now in INIF1F209
C       real*8 xxp(99)/
C     > -3.000,-2.939,-2.878,-2.816,-2.755,-2.694,-2.633,-2.571,-2.510,
C     > -2.449,-2.388,-2.327,-2.265,-2.204,-2.143,-2.082,-2.020,-1.959,
C     > -1.898,-1.837,-1.776,-1.714,-1.653,-1.592,-1.531,-1.469,-1.408,
C     > -1.347,-1.286,-1.224,-1.163,-1.102,-1.041,-0.980,-0.918,-0.857,
C     > -0.796,-0.735,-0.673,-0.612,-0.551,-0.490,-0.429,-0.367,-0.306,
C     > -0.245,-0.184,-0.122,-0.061, 0.000, 0.061, 0.122, 0.184, 0.245,
C     >  0.306, 0.367, 0.429, 0.490, 0.551, 0.612, 0.673, 0.735, 0.796,
C     >  0.857, 0.918, 0.980, 1.041, 1.102, 1.163, 1.224, 1.286, 1.347,
C     >  1.408, 1.469, 1.531, 1.592, 1.653, 1.714, 1.776, 1.837, 1.898,
C     >  1.959, 2.020, 2.082, 2.143, 2.204, 2.265, 2.327, 2.388, 2.449,
C     >  2.510, 2.571, 2.633, 2.694, 2.755, 2.816, 2.878, 2.939, 3.000/
! these are 100x bigger for convenience
c       real*8 fyp(99)/
c     > 0.0272,0.0326,0.0390,0.0464,0.0551,0.0651,0.0766,0.0898,0.1049,
c     > 0.1221,0.1416,0.1636,0.1883,0.2159,0.2466,0.2807,0.3182,0.3595,
c     > 0.4045,0.4535,0.5066,0.5637,0.6249,0.6901,0.7593,0.8324,0.9090,
c     > 0.9890,1.0720,1.1577,1.2454,1.3349,1.4254,1.5163,1.6070,1.6968,
c     > 1.7849,1.8705,1.9529,2.0313,2.1049,2.1731,2.2350,2.2901,2.3379,
c     > 2.3776,2.4090,2.4317,2.4454,2.4500,2.4454,2.4317,2.4090,2.3776,
c     > 2.3379,2.2901,2.2350,2.1731,2.1049,2.0313,1.9529,1.8705,1.7849,
c     > 1.6968,1.6070,1.5163,1.4254,1.3349,1.2454,1.1577,1.0720,0.9890,
c     > 0.9090,0.8324,0.7593,0.6901,0.6249,0.5637,0.5066,0.4535,0.4045,
c     > 0.3595,0.3182,0.2807,0.2466,0.2159,0.1883,0.1636,0.1416,0.1221,
c     > 0.1049,0.0898,0.0766,0.0651,0.0551,0.0464,0.0390,0.0326,0.0272/
...
C       real*8 xvald0(50)/
C     >  0.1964E+01, 0.1086E+01, 0.5313E-02, 0.1265E+01, 0.8000E+01,
C     >  0.2979E+00, 0.1354E+00, 0.2200E+00, 0.8296E-01, 0.9578E-01,
C     >  0.1094E+00, 0.3794E+00, 0.8122E+01, 0.5189E+01, 0.3290E+01,
C     >  0.1870E+01, 0.6110E+01,-0.3464E+02, 0.9000E+03, 0.1717E+01,
C     >  0.4335E-01, 0.1915E+03, 0.2232E+00, 0.2119E+01, 0.2088E+01,
C     > -0.3029E+00, 0.2012E+00, 0.1104E-02, 0.2276E-01,-0.4562E+00,
C     >  0.2397E+00, 0.1204E+01, 0.2321E-01, 0.5419E+03, 0.2247E+00,
C     >  0.2168E+01, 0.2266E+03, 0.7649E-01, 0.1457E+01, 0.1318E+00,
C     > -0.7534E+02, 0.1776E+00, 0.1636E+01, 0.1350E+00,-0.5596E-02,
C     >  0.5883E-02, 0.1934E+01, 0.3800E+00, 0.3319E+01, 0.1446E+00/
...
C       real*8 fyd(200)/
C     > 0.00001,0.00002,0.00003,0.00005,0.00006,0.00009,0.00010,0.00013,
C     > 0.00015,0.00019,0.00021,0.00026,0.00029,0.00034,0.00038,0.00044,
C     > 0.00049,0.00057,0.00062,0.00071,0.00078,0.00089,0.00097,0.00109,
C     > 0.00119,0.00134,0.00146,0.00161,0.00176,0.00195,0.00211,0.00232,
C     > 0.00252,0.00276,0.00299,0.00326,0.00352,0.00383,0.00412,0.00447,
C     > 0.00482,0.00521,0.00560,0.00603,0.00648,0.00698,0.00747,0.00803,
C     > 0.00859,0.00921,0.00985,0.01056,0.01126,0.01205,0.01286,0.01376,
C     > 0.01467,0.01569,0.01671,0.01793,0.01912,0.02049,0.02196,0.02356,
C     > 0.02525,0.02723,0.02939,0.03179,0.03453,0.03764,0.04116,0.04533,
C     > 0.05004,0.05565,0.06232,0.07015,0.07965,0.09093,0.10486,0.12185,
C     > 0.14268,0.16860,0.20074,0.24129,0.29201,0.35713,0.44012,0.54757,
C     > 0.68665,0.86965,1.11199,1.43242,1.86532,2.44703,3.22681,4.24972,
C     > 5.54382,7.04016,8.48123,9.40627,9.40627,8.48123,7.04016,5.54382,
C     > 4.24972,3.22681,2.44703,1.86532,1.43242,1.11199,0.86965,0.68665,
C     > 0.54757,0.44012,0.35713,0.29201,0.24129,0.20074,0.16860,0.14268,
C     > 0.12185,0.10486,0.09093,0.07965,0.07015,0.06232,0.05565,0.05004,
C     > 0.04533,0.04116,0.03764,0.03453,0.03179,0.02939,0.02723,0.02525,
C     > 0.02356,0.02196,0.02049,0.01912,0.01793,0.01671,0.01569,0.01467,
C     > 0.01376,0.01286,0.01205,0.01126,0.01056,0.00985,0.00921,0.00859,
C     > 0.00803,0.00747,0.00698,0.00648,0.00603,0.00560,0.00521,0.00482,
C     > 0.00447,0.00412,0.00383,0.00352,0.00326,0.00299,0.00276,0.00252,
C     > 0.00232,0.00211,0.00195,0.00176,0.00161,0.00146,0.00134,0.00119,
C     > 0.00109,0.00097,0.00089,0.00078,0.00071,0.00062,0.00057,0.00049,
C     > 0.00044,0.00038,0.00034,0.00029,0.00026,0.00021,0.00019,0.00015,
C     > 0.00013,0.00010,0.00009,0.00006,0.00005,0.00003,0.00002,0.00001/
C       real*8 avp2(200)/
C     >     1.0,0.98974,0.96975,0.96768,0.94782,0.94450,0.92494,0.92047,
C     > 0.90090,0.89563,0.87644,0.87018,0.85145,0.84434,0.82593,0.81841,
C     > 0.80021,0.79212,0.77444,0.76553,0.74866,0.73945,0.72264,0.71343,
C     > 0.69703,0.68740,0.67149,0.66182,0.64631,0.63630,0.62125,0.61154,
C     > 0.59671,0.58686,0.57241,0.56283,0.54866,0.53889,0.52528,0.51581,
C     > 0.50236,0.49291,0.47997,0.47063,0.45803,0.44867,0.43665,0.42744,
C     > 0.41554,0.40656,0.39511,0.38589,0.37488,0.36611,0.35516,0.34647,
C     > 0.33571,0.32704,0.31656,0.30783,0.29741,0.28870,0.27820,0.26945,
C     > 0.25898,0.25010,0.23945,0.23023,0.21943,0.20999,0.19891,0.18911,
C     > 0.17795,0.16793,0.15669,0.14667,0.13553,0.12569,0.11504,0.10550,
C     > 0.09557,0.08674,0.07774,0.06974,0.06184,0.05484,0.04802,0.04203,
C     > 0.03629,0.03129,0.02654,0.02247,0.01867,0.01545,0.01251,0.01015,
C     > 0.00810,0.00664,0.00541,0.00512,0.00512,0.00541,0.00664,0.00810,
C     > 0.01015,0.01251,0.01545,0.01867,0.02247,0.02654,0.03129,0.03629,
C     > 0.04203,0.04802,0.05484,0.06184,0.06974,0.07774,0.08674,0.09557,
C     > 0.10550,0.11504,0.12569,0.13553,0.14667,0.15669,0.16793,0.17795,
C     > 0.18911,0.19891,0.20999,0.21943,0.23023,0.23945,0.25010,0.25898,
C     > 0.26945,0.27820,0.28870,0.29741,0.30783,0.31656,0.32704,0.33571,
C     > 0.34647,0.35516,0.36611,0.37488,0.38589,0.39511,0.40656,0.41554,
C     > 0.42744,0.43665,0.44867,0.45803,0.47063,0.47997,0.49291,0.50236,
C     > 0.51581,0.52528,0.53889,0.54866,0.56283,0.57241,0.58686,0.59671,
C     > 0.61154,0.62125,0.63630,0.64631,0.66182,0.67149,0.68740,0.69703,
C     > 0.71343,0.72264,0.73945,0.74866,0.76553,0.77444,0.79212,0.80021,
C     > 0.81841,0.82593,0.84434,0.85145,0.87018,0.87644,0.89563,0.90090,
C     > 0.92047,0.92494,0.94450,0.94782,0.96768,0.96975,0.98974,1.0/
...
c       real*8 fyd(20)/ 0.4965, 0.4988, 0.4958, 0.5008, 0.5027, 0.5041,
c     >  0.5029, 0.5034, 0.4993, 0.5147, 0.5140, 0.4975, 0.5007, 0.4992,
c     >  0.4994, 0.4977, 0.5023, 0.4964, 0.4966, 0.4767/
c       real*8 avpz(20)/-0.1820,-0.0829,-0.0590,-0.0448,-0.0345,-0.0264,
c     > -0.0195,-0.0135,-0.0079,-0.0025, 0.0029, 0.0083, 0.0139, 0.0199,
c     >  0.0268, 0.0349, 0.0453, 0.0598, 0.0844, 0.1853/
c       real*8 avp2(20)/ 0.0938, 0.0219, 0.0137, 0.0101, 0.0081, 0.0068,
c     >  0.0060, 0.0054, 0.0051, 0.0049, 0.0050, 0.0051, 0.0055, 0.0060,
c     >  0.0069, 0.0081, 0.0102, 0.0140, 0.0225, 0.0964/
cc Look up tables for deuteron in fine bins for sub threshold
c       real*8 fydf(200)/
c     > 0.00001,0.00002,0.00003,0.00005,0.00006,0.00009,0.00010,0.00013,
c     > 0.00015,0.00019,0.00021,0.00026,0.00029,0.00034,0.00038,0.00044,
c     > 0.00049,0.00057,0.00062,0.00071,0.00078,0.00089,0.00097,0.00109,
...
! Do fast 20 bins if abvoe threshold
      if(w2.gt.1.30) then
       do ism = 1,20
         w2p = (amd + nu - sqrt(am**2 + avp2d(ism)))**2 - 
     >    qv**2 + 2. * qv * avpzd(ism) - avp2d(ism)
        if(w2p.gt.1.155) then
          call CHRISTY507(W2p,Q2,F1p,Rp,sigtp,siglp)
          sigt = sigt + sigtp * fydd(ism) / 10.
          sigl = sigl + siglp * fydd(ism) / 10.
          F1   = F1   + F1p   * fydd(ism) / 10.
        endif
       enddo
      else
       do ism = 1,200
...
c Need avp2f term to get right behavior x>1! 
        w2p = (amd + nu - sqrt(am**2 + avp2fd(ism)))**2 - 
     >    qv**2 + 2. * qv * pz - avp2fd(ism)
        if(w2p.gt.1.155) then
          call CHRISTY507(W2p,Q2,F1p,Rp,sigtp,siglp)
          sigt = sigt + sigtp * fydfd(ism) / 100.
          sigl = sigl + siglp * fydfd(ism) / 100.
          F1   = F1   + F1p   * fydfd(ism) / 100.
        endif
       enddo
      endif
...
C----------------------------------------------------------------------------
      SUBROUTINE EMC_09(X,A,GOODFIT,RES)
      REAL RES, FITEMC_09
      REAL X, A
      REAL XX, AA
      logical GOODFIT
